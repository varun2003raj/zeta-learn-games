  ...
  const columns = [
    {
      ...
      render: (room) => (
        <div>
          <p className="font-semibold text-slate-100">{room.title}</p>
    ...
    },
    {
      ...
      render: (room) => (
        <div className="flex flex-wrap gap-2">
          <Link
            to={`/admin/escape-rooms/${room.id}/levels`}
            className="rounded-lg border border-slate-600 px-2 py-1 text-xs text-slate-100 hover:bg-slate-800"
          >
            View
          </Link>
          <Link
            to={`/admin/escape-rooms/${room.id}/edit`}
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-slate-100">Escape Rooms</h1>
          <p className="text-sm text-slate-400">
            Search, filter, publish, and manage all rooms.
          </p>
        </div>
        <Link
          to="/admin/escape-rooms/new"
