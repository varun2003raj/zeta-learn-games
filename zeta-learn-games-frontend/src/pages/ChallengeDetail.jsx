...
export default function ChallengeDetail() {
  ...
  useEffect(() => {
    API.get("ctf/challenges/admin/ctf/state/")
  ...
  if (usedHints[hintNumber]) return;

  try {
    setHintLoading(hintNumber);

    const res = await API.post(
      `ctf/challenges/${id}/hint/`,
      { hint: hintNumber }
    );
    ...
    await showPopup(
      err?.response?.data?.error || "Unable to use hint.",
      "Hint"
    );
  } finally {
    setHintLoading(null);
  ...
  return (
    <div className={`ctf-shell public-shell min-h-screen bg-gray-950 text-white public-model-${themeModel}`}>
      ...
      <div className="p-6">
        ...
        <div className="mt-6 bg-gray-900 p-4 rounded-2xl border border-gray-800 whitespace-pre-line break-words leading-7">
  {challenge.description}
        ...
        {finished && (
          <div className="mt-6 rounded-2xl border border-red-500/40 bg-red-950/40 p-6 text-center">
            <p className="text-3xl font-bold text-red-400">⏰ TIME OVER</p>
  ...
  <div className="mt-3 bg-gray-900 p-4 rounded-2xl border border-gray-800">
    ...
    {challenge.hint_1_visible && (
      <div className="mb-4">
        {!hintMessages[1] ? (
          <button
            onClick={() => useHint(1)}
            disabled={hintLoading === 1}
            className="bg-yellow-500 text-black px-4 py-2 rounded-xl font-bold"
          >
            {hintLoading === 1
              ? "Loading..."
              : "Reveal Hint 1 (-10 points)"}
          </button>
        ) : (
          <div>
    ...
    {challenge.hint_2_visible && (
      <div>
        {!hintMessages[2] ? (
          <button
            onClick={() => useHint(2)}
            disabled={hintLoading === 2}
            className="bg-yellow-500 text-black px-4 py-2 rounded-xl font-bold"
          >
            {hintLoading === 2
              ? "Loading..."
              : "Reveal Hint 2 (-10 points)"}
          </button>
        ) : (
          <div>
            <p className="text-yellow-300">
      ...
      {!finished && (
        <div className="mt-6">
          <input
            className="p-3 rounded-xl bg-gray-800 w-[400px]"
          ...
          <button
            onClick={submitFlag}
            className="ml-3 bg-green-600 px-6 py-3 rounded-xl font-bold hover:bg-green-700"
          >
            Submit Flag
          </button>
        </div>
