...
function TreasureHuntGame() {
  ...
  const applyLevelSuccess = useCallback(
    (goldAwarded, attemptCount) => {
      console.log("SENDING TREASURE SCORE:", {
        level: currentLevel,
        score: goldAwarded,
      ...
      API.post("treasure-hunt/complete/", {
        level: currentLevel,
        score: goldAwarded,
        attempts: attemptCount,
        hints_used: hintsUsed,
      }).catch((err) => {
        console.error("Failed to save Treasure Hunt progress:", err);
      });
  ...
  const handleRestartSimulation = async () => {
    try {
      await API.post("treasure-hunt/restart/");
      ...
      setLogs([
        buildLogEntry(
          "captain",
          "Voyage restarted from the first island."
        ),
        buildLogEntry(
          "captain",
          "Crew ready to hunt the lost treasure."
        ),
      ]);
    } catch (error) {
      console.error("Failed to restart Treasure Hunt:", error);
    }
  };
