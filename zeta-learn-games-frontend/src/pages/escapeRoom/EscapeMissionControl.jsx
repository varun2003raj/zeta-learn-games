...
export default function EscapeMissionControl() {
  ...
  const loadRooms = useCallback(async () => {
    try {
      ...
      console.log(
  "ESCAPE GLOBAL PROGRESS DETAILS:",
  globalProgress.map((p) => ({
    room: p.room,
  ...
  allProgressRows.forEach((row) => {
    ...
    console.log("PROGRESS ROOM ID:", row?.room?.id);
  ...
  room.title,
  "PROGRESS:",
  globalProgressByRoom.get(String(room.id))
);
...
const previousRoomCompleted = previousRoom
  ? Boolean(
      globalProgressByRoom.get(String(previousRoom.id))?.completed
    )
  : true;

const lockedBySequence = Boolean(
  previousRoom && !previousRoomCompleted
);
...
if (progressRow?.completed) {
  ...
  const getModalThemeClass = () => {
  const title = String(selectedRoom?.title || "").toLowerCase();

  if (title.includes("jungle")) {
    return "modal-theme-jungle";
  }

  if (title.includes("volcano")) {
    return "modal-theme-volcano";
  }

  if (title.includes("ocean")) {
    return "modal-theme-ocean";
  }
      ...
      {isEntryModalOpen ? (
        ...
        >
          ...
          >
            ...
            {/* ROOM DESCRIPTION */}
              <p className="escape-entry-description game-card-text">
