...
export default function CtfLeaderboard() {
  ...
  return (
    <section className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">CTF Leaderboard</h1>
      ...
      {!timeline?.time_start ? (
        ...
        <>
          <div className="flex items-center gap-2">
            <label className="text-sm text-slate-700">Team View</label>
            <select
              value={selectedTeam}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/95 p-3">
            <svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="h-[460px] w-full">
              <rect x="0" y="0" width={chart.width} height={chart.height} fill="#f8fbff" rx="14" />
          ...
          <div className="flex flex-wrap gap-2">
            {chart.lines.map((line) => (
              <div
                key={line.name}
                className="rounded-lg border border-slate-200 bg-white/95 px-2 py-1 text-xs text-slate-800"
              >
                <span
