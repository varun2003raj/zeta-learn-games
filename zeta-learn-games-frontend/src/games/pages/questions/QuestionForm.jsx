...
export default function QuestionForm() {
  ...
  const backPath = useMemo(
    () => `/games/levels/${targetLevelId || levelId}/questions`,
    ...
    <section className="mx-auto max-w-4xl space-y-4">
      ...
      >
        <label className="space-y-1">
          <span className="text-sm text-slate-700">Hint Text</span>
          <textarea
            rows={5}
            required
            value={form.text}
        ...
        <label className="space-y-1">
          <span className="text-sm text-slate-700">Penalty Points</span>
          <input
            type="number"
            min="0"
