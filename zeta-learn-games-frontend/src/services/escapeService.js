...
const ATTEMPT_BASE = "/escape/admin/submissions/";
...
const normalizeProgress = (progress = {}, roomTitleMap = new Map()) => {
  const roomId =
    progress?.room?.id ||
    progress?.escape_room?.id ||
    progress?.room ||
    "";
  const roomTitle =
    progress?.escape_room?.title ||
    progress?.room?.title ||
  ...
  return {
    ...
    completed: toBoolean(
      progress?.completed ?? String(progress?.status || "").toLowerCase() === "completed"
    ),
...
const normalizeAttempt = (attempt = {}) => ({
  ...
  escape_room: attempt?.escape_room ?? attempt?.room ?? null,
  escape_room_id:
    attempt?.escape_room?.id ||
    attempt?.escape_room ||
    attempt?.room?.id ||
    attempt?.room ||
    null,
...
const buildPuzzlePayload = (payload = {}, partial = false) => {
  ...
  if (!partial || payload?.challenge !== undefined) {
    nextPayload.challenge = String(payload?.challenge || "").trim();
  }
...
const escapeService = {
  ...
  async getPlayableRoom(roomId) {
    ...
    console.log("PUZZLE 1:", response.data.puzzles?.[0]);
  ...
  async startEscape({ roomId }) {
  const response = await api.post(PLAYER_START_BASE, {
    room_id: roomId,
  });
  ...
  async listMyProgress({ roomId = "", all = true } = {}) {
    ...
    console.log("RAW ALL PROGRESS RESPONSE:", response.data);
  ...
  console.log("GET ROOM RAW RESPONSE:", response.data);

  return normalizeRoom(response.data);
},

  async updateRoom(roomId, payload) {
  console.log("UPDATE ROOM CALLED:", roomId, payload);
  ...
  const response = await api.get("/escape/admin/attempts/", {
    params,
  });
  ...
  if (completed === "completed") {
    ...
    rows = rows.filter(
      (row) => String(row.status).toUpperCase() === "IN_PROGRESS"
    );
  ...
  const [rooms, puzzles, hints, allProgressRows] = await Promise.all([
    this.listAllRooms(),
    this.listAllPuzzles(),
    this.listAllHints(),
  ...
  const progressRows = allProgressRows.filter(
    (row) => row.started_at || row.completed || row.score > 0
  );
